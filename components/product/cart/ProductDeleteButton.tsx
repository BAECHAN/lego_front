...
export default function ProductDeleteButton(props: { product: ProductCartT }) {
  ...
  const delCartAPI = useMutation(
    async (param: ProductDeleteCartSubmitT) => {
      return await axios.patch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/del-cart`, JSON.stringify(param), {
    ...
    },
    {
      onSuccess: (response, variables) => {
        if (response.status === 204) {
          setSelectedOrder(selectedOrder.filter((item) => item !== variables.cart_id))
  ...
  return (
    ...
    >
      <FontAwesomeIcon icon={faTrashCan} width="15px" className="text-gray-500 hover:cursor-pointer hover:text-black" />
    </button>
  )
}
